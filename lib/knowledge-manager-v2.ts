...
export class KnowledgeManagerV2 {
  ...
  public async loadSeedData(): Promise<void> {
    try {
      ...
      // Process vocabulary seed with alphabet awareness
      if (vocab) {
        Object.entries(vocab).forEach(([word, data]: [string, any]) => {
          ...
          const entry: VocabularyEntry = {
            ...
            alphabetPosition: data.alphabet_position,
            letterType: data.letter_type,
            phoneticSound: data.phonetic_sound,
            spellingImportance: data.spelling_importance,
          }

          this.learnedKnowledge.vocabulary.set(word, entry)
        })
        console.log(`✅ Processed ${Object.keys(vocab).length} vocabulary entries including alphabet`)
      }
  ...
  private async loadSeedFile(path: string): Promise<any> {
    try {
      console.log(`Loading seed file: ${path}`)
      ...
      console.log(`✅ Successfully loaded ${path} with ${Object.keys(data).length} entries`)
      return data
    } catch (error) {
  ...
  public searchKnowledge(query: string): any[] {
    ...
    try {
      // Search vocabulary with safe access
  ...
  public async optimizeKnowledge(): Promise<void> {
    try {
      ...
      for (const [word, entry] of this.learnedKnowledge.vocabulary) {
        if (!vocabMap.has(word.toLowerCase())) {
          vocabMap.set(word.toLowerCase(), entry)
        }
      }
      this.learnedKnowledge.vocabulary = vocabMap

      // Remove duplicate math concepts
      const mathMap = new Map<string, MathEntry>()
      for (const [concept, entry] of this.learnedKnowledge.mathematics) {
        if (!mathMap.has(concept.toLowerCase())) {
          mathMap.set(concept.toLowerCase(), entry)
        }
      }
      this.learnedKnowledge.mathematics = mathMap

      // Remove old conversations (keep only last 50)
      ...
      console.error("❌ Knowledge optimization failed:", error)
      throw error
    }
  }
